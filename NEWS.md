...
- For some reason the `st_voronoi` of `sf` generates invalid geometric representations of cells in the Voronoi diagram (e.g., cells with self-intersections). Unfortunately, it can introduce several problems when processing geometric operations in `pgeometry` objects built using the construction policy based on the Voronoi diagram. Hence, we introduce a function to regularize invalid cells generated by the Voronoi diagram.
