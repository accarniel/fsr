...
fuzzy_clustering_policy <- function(tbl, k, method = "cmeans", use_coords = FALSE, iter = 100, ...) {
  ...
  if(k <= 1) {
    stop("The value of k should be greater than 1")
  }
  ...
  if(method == "cmeans") {
    ...
    stop(paste0("The method '", method,"' is not a supported fuzzy clustering algorithm. The options are either 'cmeans' or 'cshell'"))
  }
...
voronoi_diagram_policy <- function(lp, base_poly = NULL, ...) {
  ...
  for(class in cls){
    ...
    pgeom_objs <- append(pgeom_objs, spa_add_component(create_empty_pgeom("plateau_region"), lcomps))
  }
...
spa_creator <- function(tbl, fuzz_policy = "fsp", const_policy = "voronoi", ...) {
  ...
  result <- switch (const_policy,
    ...
    stop(paste0("The construction policy '", const_policy, "' is not a supported policy. 
                                   The possible values are 'voronoi' and 'delaunay'."))
    )
